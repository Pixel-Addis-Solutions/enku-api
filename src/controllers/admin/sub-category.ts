...
export const createSubCategory = async (req: Request, res: Response) => {
  try {
    ...
    const subCategory = subCategoryRepository.create({ name, description, category: { id: categoryId } });
...
export const getSubCategoryById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const subCategoryRepository = getRepository(SubCategory);
    const subCategory = await subCategoryRepository.findOne({where: {id},relations:["category"] });
    ...
    return ResUtil.internalError({
      res,
      message: "Error retrieving subcategory by ID",
      data: error,
    });
  }
};
